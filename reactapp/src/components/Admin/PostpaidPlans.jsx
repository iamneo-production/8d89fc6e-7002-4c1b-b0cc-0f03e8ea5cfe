      ...
      <div>
        ...
        {filteredPlans.length === 0 ? (
          ...
          filteredPlans.map((allPlans) => {
            if (allPlans.planType.toLowerCase() === "postpaid") {
              return (
                <PostpaidPlansItems
                  key={allPlans.planId}
                  id={allPlans.planId}
...
{filteredPlans.length > 0 && !filteredPlans.map((plan) => plan.planType === "prepaid") && (
          <div className="no-plans-message">No Postpaid plans found.</div>
        )}
